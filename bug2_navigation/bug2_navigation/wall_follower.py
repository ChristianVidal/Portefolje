...
class WallFollowerClass(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(SetBool, 'wall_follower_service', self.wall_follower_switch_callback)
    ...
    def wall_follower_switch_callback(self, request, response):
        ...
        if self.active:
            ...
            response.message = "Wall follower activated"
        else:
            response.success = True
            response.message = "Wall follower deactivated"
        return response
